...
def target_score(logits, aspect):
    ...
    if aspect == 'aspect':
        ...
        # weight = preds**1.5 #/ torch.sum(preds, dim=0)
    else:
        weight = preds**2 / torch.sum(preds, dim=0)
...
if __name__ == "__main__":
    ...
    w_emb_file = 'emb1_'+aspect + '_g2.5_pre2_iter5_w.txt'
    t_emb_file = 'emb1_'+aspect + '_g2.5_pre2_iter5_t.txt'
    ...
    for aspect in aspects:
        w_emb_file = 'emb_'+aspect + '_w_kw_g2.5_i5_w.txt'
        t_emb_file = 'emb_'+aspect + '_w_kw_g2.5_i5_t.txt'
    ...
    old_pred_senti = []
    temperature = 20.0
    with open('pred_laptop_mix.txt','w') as fout:
        ...
        for k,s in enumerate(text):
            ...
            # joint_dot_sum['aspect'] = np.zeros_like(joint_dot)
            # joint_dot_sum['senti'] = np.zeros_like(joint_dot)
    ...
    if self_training:
        ...
        for i in range(total_steps):
            if i % update_interval == 0:
                ...
                r = recall_score(np.array(gt1), aspect_pred_labels.numpy(), average='macro')
                f1_mac = f1_score(np.array(gt1), aspect_pred_labels.numpy(), average='macro')
                print(f"Aspect Acc: {aspect_test_acc * 100:.1f}% P: {p * 100:.1f}% R: {r * 100:.1f}% mac-F1: {f1_mac * 100:.1f}%")
                
                p = precision_score(1-np.array(gt2), senti_pred_labels.numpy(), average='macro')
                r = recall_score(1-np.array(gt2), senti_pred_labels.numpy(), average='macro')
                f1_mac = f1_score(1-np.array(gt2), senti_pred_labels.numpy(), average='macro')
                print(f"Senti Acc: {senti_test_acc * 100:.1f}% P: {p * 100:.1f}% R: {r * 100:.1f}% mac-F1: {f1_mac * 100:.1f}%")
                # print(f'Pseudo Apsect Acc: {pseudo_aspect_test_acc * 100:.1f}% ')
                # print(f'Pseudo Senti Acc: {pseudo_senti_test_acc * 100:.1f}% ')
                
                aspect_current_text, aspect_labels = high_conf_train_loader(all_text, torch.tensor(gt1), aspect_pred_distribution, len(marginal_topic_emb['aspect']))
                senti_current_text, senti_labels = high_conf_train_loader(all_text, 1-torch.tensor(gt2), senti_pred_distribution, len(marginal_topic_emb['senti']))
                
            if cur_idx + batch_size < len(aspect_current_text):
                select_idx = list(range(cur_idx, cur_idx + batch_size))
            else:
                select_idx = list(range(cur_idx, len(aspect_current_text))) + list(range((cur_idx + batch_size) % len(aspect_current_text)))
            cur_idx = (cur_idx + batch_size) % len(aspect_current_text)
            
            text = aspect_current_text[select_idx]
            aspect_target = aspect_labels[select_idx]

            if cur_idx1 + batch_size < len(senti_current_text):
                select_idx = list(range(cur_idx1, cur_idx1 + batch_size))
            else:
                select_idx = list(range(cur_idx1, len(senti_current_text))) + list(range((cur_idx1 + batch_size) % len(senti_current_text)))
            cur_idx1 = (cur_idx1 + batch_size) % len(senti_current_text)
            
            text = senti_current_text[select_idx]
            senti_target = senti_labels[select_idx]
            
            batch_train_func(text, aspect_target, aspect_model, 'aspect')
            batch_train_func(text, senti_target, senti_model, 'senti')
